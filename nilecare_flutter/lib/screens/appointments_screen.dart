...
class _AppointmentBookingScreenState extends State<AppointmentBookingScreen> {
  ...
  Future<void> _loadDoctors(String specialtySlug) async {
    ...
    try {
      final doctors = await _doctorService.getDoctors(specialtySlug: specialtySlug);
      setState(() {
        _doctors = doctors;
          ...
          children: [
            ...
            loadingSpecialties
                ...
                : _buildDropdown(
                    ...
                    _specialties.map((s) => s['name'] as String).toList(),
                    selectedSpecialty,
                    (val) {
                      setState(() => selectedSpecialty = val);
                      final slug = _specialties.firstWhere(
                          (s) => s['name'] == val)['slug'] as String;
                      _loadDoctors(slug);
                    },
                    dropdownTextColor,
                    inputFillColor,
            ...
            loadingDoctors
                ...
                : _doctors.isEmpty
                    ...
                    : _buildDropdown(
                        ...
                        _doctors.map((d) => d.name).toList(),
                        selectedDoctor?.name,
                        (val) {
                          final doctor = _doctors.firstWhere((d) => d.name == val);
  ...
  // 🟢 Existing widget builders (_buildDropdown, _buildCalendar, _buildTimePicker, _buildNoteInput) remain same
